...
fname="$(basename "$DATA_PATH")"      # e.g. data.feather or data.csv
base="${fname%.*}"                   # strips everything after the last “.”
...
if [ "$GPU_PER_INST" -gt 0 ]; then
  ...
  for i in $(seq 0 $((TOTAL_INSTANCES - 1))); do
    ...
    rm -f "API_MULTI_GPU_${GPU_LIST}_INS${i}_${base}.log"
    TRANSFORMERS_CACHE="$HF_CACHE" HF_HOME="$HF_CACHE" CUDA_VISIBLE_DEVICES="$GPU_LIST" \
      vllm serve "$MODEL" \
                 ...
                 --enforce-eager \
                 > "API_MULTI_GPU_${GPU_LIST}_INS${i}_${base}.log" 2>&1 &
  done
else
  ...
  for gpu_index in $(seq 0 $((NUM_GPUS - 1))); do
    for ins_index in $(seq 0 $((INST_PER_GPU - 1))); do
      rm -f "1_API_GPU${GPU}_INS${ins_index}_${base}.log"
      ...
      TRANSFORMERS_CACHE="$HF_CACHE" HF_HOME="$HF_CACHE" CUDA_VISIBLE_DEVICES="$GPU" \
        vllm serve "$MODEL" \
                   ...
                   --gpu_memory_utilization "$GPU_MEM" \
                   --enforce-eager \
                   > "1_API_GPU${GPU}_INS${ins_index}_${base}.log" 2>&1 &
