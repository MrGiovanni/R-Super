...
from .trans_layers import TransformerBlock, LayerNorm, PreNorm, Mlp
...
class BidirectionAttention(nn.Module):
    def __init__(self, feat_dim, map_dim, out_dim, heads=4, dim_head=64, attn_drop=0., proj_drop=0.,
        ...
        self.map_qv = nn.Linear(map_dim, self.inner_dim*2, bias=False)
        self.map_out = nn.Linear(self.inner_dim, map_dim, bias=False)
    ...
    def forward(self, feat, semantic_map):
        ...
        B, N, _ = semantic_map.shape

        feat_q, feat_v = self.feat_qv(feat).chunk(2, dim=1) # B, inner_dim, D, H, W
        map_q, map_v = self.map_qv(semantic_map).chunk(2, dim=2) # B, ms*ms*ms, dim
        ...
        map_q, map_v = map(lambda t: rearrange(t, 'b n (dim_head heads) -> b heads n dim_head', dim_head=self.dim_head, heads=self.heads, n=N, b=B), [map_q, map_v])
        ...
        map_out = rearrange(map_out, 'b heads n dim_head -> b n (dim_head heads)', n=N)
...
class BidirectionAttentionBlock(nn.Module):
    def __init__(self, feat_dim, map_dim, out_dim, heads, dim_head, norm=nn.BatchNorm3d, act=nn.ReLU,
        ...
        if proj_type == 'linear':
            self.feat_ffn = FusedMBConv(out_dim, out_dim, expansion=expansion, kernel_size=1, act=act, norm=norm)
        else:
            self.feat_ffn = MBConv(out_dim, out_dim, expansion=expansion, kernel_size=kernel_size, act=act, norm=norm)

        self.map_ffn = PreNorm(out_dim, Mlp(out_dim, out_dim*expansion, drop=proj_drop))
    ...
    def forward(self, x, semantic_map):
        ...
        out += self.shortcut(x)
        out = self.feat_ffn(out)

        mapp += semantic_map
        mapp = self.map_ffn(mapp) + mapp
...
class SemanticMapGeneration(nn.Module):
    ...
    def forward(self, x):
        ...
        semantic_map = torch.einsum('bij,bkj->bik', feat, weight_map) # B, map_dim, map_code_num

        return semantic_map.permute(0, 2, 1) # B, map_code_num, map_dim
...
class SemanticMapFusion(nn.Module):
    def __init__(self, in_dim_list, dim, heads, depth=1, norm=nn.BatchNorm3d, attn_drop=0., proj_drop=0.):
        ...
        for in_dim in in_dim_list:
            self.in_proj.append(nn.Linear(in_dim, dim, bias=False))
        ...
        for in_dim in in_dim_list:
            self.out_proj.append(nn.Linear(dim, in_dim, bias=False))

    def forward(self, map_list):
        B, N, dim = map_list[0].shape
        proj_maps = [self.in_proj[i](map_list[i]) for i in range(len(map_list))]
        #B, L, C where L=DHW
        ...
        maps_out = [self.out_proj[i](attned_maps[i]) for i in range(len(map_list))]
...
class up_block(nn.Module):
    def __init__(self, in_ch, out_ch, conv_num, trans_num, up_scale=[2,2,2], kernel_size=[3,3,3], 
        ...
        map_dim = out_ch if map_dim is None else map_dim
        if map_shortcut:
            self.map_reduction = nn.Linear(in_ch+out_ch, map_dim, bias=False)
        else:
            self.map_reduction = nn.Linear(in_ch, map_dim, bias=False)
    ...
    def forward(self, x1, x2, map1, map2=None):
        ...
        if self.map_shortcut and map2 is not None:
            semantic_map = torch.cat([map1, map2], dim=2)
        else:
            semantic_map = map1
