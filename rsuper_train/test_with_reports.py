...
def detection(tumor_mask, organ_mask, spacing, th=0.5, erode=True):
    """
    ...
    if erode:
        ...
        # Dilate
        array = ndimage.binary_dilation(array, structure=np.ones((3, 3, 3)), iterations=2)
        # Multiply
        array *= original
...
def _process_single_file(file, outputs_folder, ct_folder, th):
    """
    ...
    # Prepare row
    row = {"BDMAP_ID": _normalize_id(file)}

    # Load the CT to get spacing
    ct_path = os.path.join(ct_folder, file.replace('.npz',''))
    ...
    folder = os.path.join(outputs_folder, file, 'predictions')
    ...
    for f in os.listdir(folder):
        for s in suffixes:
            if f.endswith(s):
                ...
                # Keep the original logic for organ_mask_path
                organ_mask_path = None
                volume = detection(tumor_mask_path, organ_mask_path, spacing, th)
...
def process_outputs(args, outputs_folder, ct_folder, th, workers=10, continue_processing=False, cases=None):
    ...
    first_row = _process_single_file(files_to_process[0], outputs_folder, ct_folder, th)
    csv_columns = sorted(list(first_row.keys()))
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        future_map = {
            executor.submit(_process_single_file, f, outputs_folder, ct_folder, th): f
        ...
        try:
            for future in as_completed(future_map):
                ...
                try:
                    row = future.result()
                    if row is not None:
                        write_csv_row(row, csv_columns, csv_file_path)
                        print(row)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--th", type=float, default=0.5,
                        help="Threshold for binarizing the tumor mask (default=0.5)")
