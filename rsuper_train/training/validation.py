...
def validation(net, dataloader, args, matcher=None):
    net.eval()
    ...
    with torch.no_grad():
        ...
        for (images, labels, spacing) in iterator:
            ...
            #_, label_pred = torch.max(pred, dim=1)#NO, DONT DO THAT FOR MULTI-LABEL CASES. 
            ...
            if matcher is not None:
                print('shapes:', label_pred.shape, labels.shape)
                out_ids, label_ids = matcher(label_pred, labels)
                label_pred=label_pred[out_ids]
